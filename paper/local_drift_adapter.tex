...
Concurrently, \citet{yoon2025embedding} proposed \emph{Embedding-Converter}, a unified framework for cross-model embedding transformation.
While their work focuses on a general-purpose translation architecture, we explicitly target the \emph{non-uniformity} of drift via a mixture-of-experts approach, demonstrating that local specialization outperforms global mappings on heterogeneous transformations.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/performance_vs_k.pdf}
\caption{Recall@1 vs.\ number of clusters $K$ for local affine adapters on two model pairs.  Performance improves monotonically up to $K \approx 32$, then degrades as per-cluster training data becomes insufficient.  The gain from local adaptation is much larger on the harder cross-family + cross-dimensional pair (right).}
...
Figure~\ref{fig:drift_manifold} illustrates this on a synthetic example: three embedding regions experience qualitatively different transformations (rotation, non-linear warp, and translation), producing spatially clustered residuals that a single global map cannot resolve.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/drift_manifold.pdf}
\caption{Synthetic illustration of spatially correlated drift.  Points are colored by the residual norm of a global Procrustes adapter.  Different regions experience qualitatively different transformations, producing clustered error patterns that motivate local adaptation.}
\label{fig:drift_manifold}
\end{figure}

The monotonic improvement with $K$ (Tables~\ref{tab:sweep_crossfam}--\ref{tab:sweep_hard} and Figure~\ref{fig:performance_vs_k}) provides indirect evidence: if drift were spatially uncorrelated, increasing $K$ would not help, since each cluster's training set would contain an arbitrary mix of drift directions.
