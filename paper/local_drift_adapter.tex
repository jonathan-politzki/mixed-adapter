...
\begin{equation}
  A(z) = \sum_{k=1}^{K} w_k(z) \cdot A_k(z).
  \label{eq:moe}
\end{equation}
...
\begin{equation}
  \phi_i = \Bigl[\,\overline{\fold(x_i)}\;,\; \alpha \cdot \overline{d_i}\,\Bigr] \in \Rbb^{d_{\mathrm{old}} + d_{\mathrm{new}}},
  \label{eq:combined}
\end{equation}
where $\overline{v} = v / \|v\|_2$ denotes $\ell_2$-normalization and $\alpha > 0$ is a hyperparameter controlling the relative importance of drift information.
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
  ...
  \item \textbf{MRR@10}: mean reciprocal rank of the first relevant document.
...
We use FAISS \citep{johnson2019faiss} for $k$-means clustering and nearest-neighbor retrieval.
Training uses Adam with learning rate $10^{-3}$, batch size 256, for up to 100 epochs with early stopping (patience 5) on validation cosine similarity.
...
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{Model Pair} & \textbf{Adapter} & \textbf{R@10} & \textbf{R@100} & \textbf{MRR@10} \\
\midrule
...
\begin{tabular}{@{}ccccc@{}}
\toprule
$K$ & \textbf{R@1} & \textbf{R@10} & \textbf{R@100} & \textbf{MRR@10} \\
\midrule
